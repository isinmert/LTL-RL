...
from utils import use_flie, ltl2dot, use_pytool
...
parser.add_argument("--max_depth", type=int,
    help="Maximum depth of pytools solver default is 8")
parser.add_argument("-print_traces", default=False, action="store_true")
parser.add_argument("-show_reward", default=False, action="store_true")
parser.add_argument("-tqdm_off", default=False, action="store_true",
    help="Stop tqdm")
...
parser.add_argument("-disable_until", dest="disable_until", action="store_true",
    help='Disable until operator')
parser.set_defaults(disable_until=False, max_depth=None)
...
ALLOW_UNTIL = not args.disable_until
...
for i in tqdm(range(NUM_EPISODES), disable=args.tqdm_off):
    ...
    if (len(pos_traces_imp) > 0 and len(neg_traces_imp) > 0
        ...
        output_pytool = use_pytool(trace_dir)
        new_formula_pytool = output_pytool[0]
        new_formula_gen = flie2genLTL(new_formula_pytool, predicate_list)
        fileformula.write(new_formula_gen+"\n")
        print(new_formula_pytool)
